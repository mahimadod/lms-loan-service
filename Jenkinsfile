pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = "mahimadod/lms-loan-service"
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                git branch: 'master', url: 'https://github.com/mahimadod/lms-loan-service.git'
            }
        }
        ...
        stage('Deploy') {
            steps {
                ...
                bat """
                    docker rm -f lms-loan || exit 0
                    docker run -d --name lms-loan --network lms-network -p 8083:8083 ${DOCKER_IMAGE}:${BUILD_NUMBER}
                """
            }
        }
    }
